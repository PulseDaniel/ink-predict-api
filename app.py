# app.py — hard group-restricted predictor (Extender always allowed)
...
# Short names → your DC21 ink names
ALIASES = {
    ...
    # If you have a real Warm Red base, replace this with its true code.
...
def to_dc_names(short_list):
    ...
    for s in short_list:
        name = ALIASES.get(s)
        if name and name in base_cols:  # only if model actually has this base
            out.append(name)
    return sorted(set(out))

GROUPS = {g: to_dc_names(names) for g, names in _GROUPS_SHORT.items()}
ALL_GROUP_NAMES = sorted(list(GROUPS.keys()))

def allowed_inks_for_group(group: Optional[str]) -> set:
    """Return set of allowed ink names for the group (Extender always allowed). If group is None/empty, allow all."""
    if not group:
    ...
    for ex in extenders:
        for b in base_cols:
            if ex.lower() in b.lower():
                allowed.add(b)
    # If mapping resolved to nothing, allow extenders-only (safety).
    return allowed if allowed else {b for b in base_cols if any(ex.lower() in b.lower() for ex in extenders)}
...
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],  # lock this to your Pages origin later
...
def predict(inp: PredictIn):
    ...
    # ----- HARD GROUP FILTER on selection -----
    ...
    # optional neighbour lock (won’t reintroduce banned inks; we re-mask anyway)
    if inp.lock_neighbours:
        y = lock_to_neighbor_inkset(L, a, b, base_cols, y, lib_df, k=3, de_gate=3.0, min_pct=0.5)

    # safety: re-apply HARD GROUP FILTER to final percentages
